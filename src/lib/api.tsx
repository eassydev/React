...
export const bulkUploadWolooCategories = async (file: File) => {
  ...
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.post('/woloo/categories/bulk-upload', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'admin-auth-token': token || '',
      },
...
// Woloo Subcategories
export const fetchWolooSubcategories = async (page = 1, size = 50, status = 'all', search = '', categoryId = '') => {
  try {
    const token = getToken();
...
export const bulkUploadWolooSubcategories = async (file: File) => {
  ...
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.post('/woloo/subcategories/bulk-upload', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'admin-auth-token': token || '',
      },
...
export const fetchWolooAttributeOptions = async (attributeId: string) => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.get(`/woloo/attributes/${attributeId}/options`, {
      headers: {
        'admin-auth-token': token || '',
      },
    });
...
export const createWolooAttributeOption = async (attributeId: string, optionData: WolooAttributeOption) => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.post(`/woloo/attributes/${attributeId}/options`, optionData, {
      headers: {
        'Content-Type': 'application/json',
        'admin-auth-token': token || '',
      },
...
export const updateWolooAttributeOption = async (attributeId: string, optionId: string, optionData: WolooAttributeOption) => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.put(`/woloo/attributes/${attributeId}/options/${optionId}`, optionData, {
      headers: {
        'Content-Type': 'application/json',
        'admin-auth-token': token || '',
      },
...
export const deleteWolooAttributeOption = async (attributeId: string, optionId: string) => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.delete(`/woloo/attributes/${attributeId}/options/${optionId}`, {
      headers: {
        'admin-auth-token': token || '',
      },
    });
...
export const bulkUploadWolooAttributes = async (file: File) => {
  try {
    ...
    const response: AxiosResponse = await apiClient.post('/woloo/attributes/bulk-upload', formData, {
      headers: {
        'admin-auth-token': token || '',
        'Content-Type': 'multipart/form-data',
      },
...
export const exportWolooAttributes = async (format: 'excel' | 'csv' = 'excel') => {
  try {
    ...
    const link = document.createElement('a');
    link.href = url;
    const filename = format === 'csv' ? 'woloo_attributes_export.csv' : 'woloo_attributes_export.xlsx';
    link.setAttribute('download', filename);
    document.body.appendChild(link);
    link.click();
    link.remove();
...
export const fetchWolooCategorySubcategoryIds = async () => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.get('/woloo/attributes/category-subcategory-ids', {
      headers: {
        'admin-auth-token': token || '',
      },
    });
...
// Woloo Rate Cards
export const fetchWolooRateCards = async (page = 1, size = 50, status = 'all', search = '', categoryId = '', subcategoryId = '') => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.get('/woloo/rate-cards', {
      ...
      params: { page, size, status, search, category_id: categoryId, subcategory_id: subcategoryId },
    });
    return response.data;
  } catch (error) {
...
export const exportWolooRateCards = async (format: 'excel' | 'csv' = 'excel') => {
  try {
    const token = getToken();
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/woloo/rate-cards/export?format=${format}`, {
      headers: {
        'admin-auth-token': token || '',
      },
    });
...
export const bulkUploadWolooRateCards = async (file: File) => {
  ...
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.post('/woloo/rate-cards/bulk-upload', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'admin-auth-token': token || '',
      },
...
// Woloo Bookings
export const fetchWolooBookings = async (page = 1, size = 50, status = '', search = '', dateFrom = '', dateTo = '') => {
  try {
    const token = getToken();
...
export const updateWolooBookingStatus = async (id: string, statusData: { status: string; reason?: string; notes?: string }) => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.put(`/woloo/bookings/${id}/status`, statusData, {
      headers: {
        'Content-Type': 'application/json',
        'admin-auth-token': token || '',
      },
...
export const assignWolooBookingProvider = async (id: string, providerData: { provider_id: string; assignment_notes?: string; notify_provider?: boolean }) => {
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.put(`/woloo/bookings/${id}/provider`, providerData, {
      headers: {
        'Content-Type': 'application/json',
        'admin-auth-token': token || '',
      },
...
export const rescheduleWolooBooking = async (id: string, rescheduleData: {
  booking_date: string;
  start_service_time: string;
  end_service_time: string;
  ...
  try {
    const token = getToken();
    const response: AxiosResponse = await apiClient.put(`/woloo/bookings/${id}/reschedule`, rescheduleData, {
      headers: {
        'Content-Type': 'application/json',
        'admin-auth-token': token || '',
      },
...
export const assignWolooBookingStaff = async (id: string, staffData: {
...
export const downloadWolooCategorySampleCSV = async () => {
  try {
    ...
    throw new Error(error.response?.data?.message || 'Failed to download Woloo category sample CSV.');
  }
};
...
export const downloadWolooSubcategorySampleCSV = async () => {
  try {
    ...
    throw new Error(error.response?.data?.message || 'Failed to download Woloo subcategory sample CSV.');
  }
};
