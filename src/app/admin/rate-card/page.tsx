...
import { fetchRateCards, deleteRateCard } from '@/lib/api';
...
const RateCardList = () => {
  ...
  // Fetch rate cards from the backend with pagination
  const fetchRateCardsData = async (page = 1, size = 5) => {
    try {
      const { data, meta } = await fetchRateCards(page, size);
      setRateCards(data);
  ...
  const handleRateCardDelete = async (rateCard: any) => {
    try {
      await deleteRateCard(rateCard.id);
      toast({
        title: 'Success',
        description: `Rate Card "${rateCard.name}" deleted successfully`,
        variant: 'success',
      });
      fetchRateCardsData(pagination.pageIndex + 1, pagination.pageSize);
    } catch (error) {
      toast({
        title: 'Error',
        description: `Failed to delete rate card: ${error}`,
        variant: 'destructive',
      });
    }
  };
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <p>Are you sure you want to delete rate card: {row.original.name}?</p>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <Button variant="secondary" onClick={() => handleRateCardDelete(row.original)}>
                  Yes, Delete
                </Button>
