...
import { importSubcatgeory } from '@/lib/api';
...
const ImportRateCardForm = () => {
  ...
  const handleImport = async () => {
    ...
    try {
      setIsImporting(true);
      await importSubcatgeory(file);
      toast({
        title: 'Success',
        description: 'subcategory  imported successfully.',
        variant: 'success',
      });
      //   setFile(null);
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Failed to import rate cards.',
        variant: 'destructive',
      });
    } finally {
      ...
      <div className="max-w-3xl mx-auto">
        <Card className="shadow-xl bg-white/80 backdrop-blur">
          <CardHeader className="border-b border-gray-100 pb-4">
            <CardTitle className="text-xl text-gray-800">Import Sub category</CardTitle>
          </CardHeader>
          <CardContent>
