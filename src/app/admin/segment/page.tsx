...
const ServiceSegmentList = () => {
  ...
  const handleServiceSegmentDelete = async (serviceSegmentId: string) => {
    // ID is a string
    try {
      ...
      cell: ({ row }) => (
        <div className="flex items-center space-x-2">
          <Button variant="ghost" size="icon">
            <Link href={`/admin/segment/edit/${row.original.id}`} passHref>
              {' '}
              {/* Correct link */}
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                <Button
                  variant="secondary"
                  onClick={() => handleServiceSegmentDelete(row.original.id)}
                >
                  {' '}
                  {/* Pass ID */}
                  Yes, Delete
