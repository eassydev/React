...
import { Download, Plus, Edit, Trash2, Printer, Copy, Upload, ChevronDown } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const WolooRateCardList = () => {
  ...
  const fetchRateCardsData = async (
    page = 1,
    size = 50,
    status = 'all',
    search = '',
    ...
    try {
      const { data, meta } = await fetchWolooRateCards(
        page,
        size,
        status,
        search,
  ...
  const handleCopy = () => {
    const formattedData = rateCards
      ...
      .join('\n');
    const header =
      'ID, Name, Category, Subcategory, Price, Strike Price, Weight, Recommended, Best Deal, Status\n';
  ...
  const handleBulkUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to upload rate cards.',
        variant: 'destructive',
      });
    } finally {
  ...
  const rateCardColumns: ColumnDef<WolooRateCard>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span
          className={`badge px-2 py-1 rounded ${row.original.recommended ? 'bg-blue-200 text-blue-800' : 'bg-gray-200 text-gray-800'}`}
        >
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span
          className={`badge px-2 py-1 rounded ${row.original.best_deal ? 'bg-orange-200 text-orange-800' : 'bg-gray-200 text-gray-800'}`}
        >
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <p className="text-xl font-bold">
                  Are you sure you want to delete rate card: {row.original.name}?
                </p>
              </AlertDialogHeader>
              <AlertDialogFooter>
